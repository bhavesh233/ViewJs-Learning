...
const store = createStore({
  ...
  mutations: {
    ...
    deleteData(state, payload) {
      const editData = state.listData.splice(payload, 1);
      for (let fetchData in state.listData) {
        if (editData === fetchData) {
          state.listData.splice(fetchData, 1);
        }
      }
      
    },
    edit(state, payload) {
      // const edit = state.listData.find((s) => {
      //   return s.id === payload.id;
      // });

      state.listData[payload.id] = payload;
    },
  },
  actions: {},
  getters: {
